...
public class MyContour implements GeoServerProcess {
   ...
   * 
   * @param interval
   *          the interval between contour values (if {@code levels} is not provided)
   * 
   * @param simplify
   *          whether to simplify contour lines by removing co-linear vertices; default is to simplify
   * 
   * @param smooth
   *          whether to apply Bezier smooth to the contours; default is no smoothing
   * 
   * @param roi
   *          an optional polygonal {@code Geometry} to define the region of interest within which contours will be generated
   * 
   * @return the contours a feature collection where each feature contains a contour as a {@linkplain LineString} and the contour value as a {@code Double}
   * 
